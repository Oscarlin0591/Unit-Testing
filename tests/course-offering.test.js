...
describe("CourseOffering", () => {
  ...
  describe("constructor", () => {
    ...
    test("instructor defaults to null", () =>
      expect(offering.instructor).toBeNull());
    test("registeredStudents defaults to empty array", () =>
      expect(offering.registeredStudents).toEqual([]));
    test("grades defaults to empty object", () =>
  ...
  describe("submit_grade", () => {
    ...
    test("returns error message for invalid grade", () => {
      expect(offering.submit_grade(student, "Z")).toBe(
        "Please enter a valid grade",
      );
    });

    test("returns error message when argument is not a Student", () => {
      expect(offering.submit_grade("not-a-student", "A")).toBe(
        "Please enter a valid grade",
      );
    });
  });
  ...
  describe("toString", () => {
    test("without instructor shows course, section, quarter, and year", () => {
      expect(offering.toString()).toBe(
        "Software Engineering, CS 330-1 (Fall 2024)",
      );
    });
    ...
    test("with instructor includes instructor name", () => {
      ...
      expect(offering.toString()).toBe(
        "Software Engineering, CS 330-1, Chris Brown (Fall 2024)",
      );
    });
  });
});
